...
func Run(paths []string) {
	...
	// run director
	{
		...
		{
			menuView := &MenuView{director: d, paths: paths}	
		...
		for !d.window.ShouldClose() {
			{
				...
				if d.view != nil {
					...
					case *GameView:
						...
						if joystickReset(glfw.Joystick1) || joystickReset(glfw.Joystick2) || readKey(v.director.window, glfw.KeyEscape) {
							setView(v.director, d.menuView)
						}
						// update controllers
						{
							turbo := v.console.PPU.Frame%6 < 3
							k1 := readKeys(v.director.window, turbo)
						...
						// draw buffer
						{
							w, h := v.director.window.GetFramebufferSize()
							s1 := float32(w) / 256
							s2 := float32(h) / 240
							f := float32(1 - padding)
							var x, y float32
					...
					case *MenuView:
						// check buttons
						{
							k1 := readKeys(v.director.window, false)
								...
								if buttons[i] && !v.buttons[i] {
									...
									case nes.ButtonStart:
										...
										if index < len(v.paths) {
											playGame(v.director, v.paths[index])
										}
									}
...
func clampScroll(v *MenuView, wrap bool) {
	n := len(v.paths)
	rows := n / v.nx
	if n%v.nx > 0 {
		rows++
...
func setView(d *Director, view View) {
	if d.view != nil {
		switch v:= d.view.(type) {
		case *GameView:
	...
	if d.view != nil {
		...
		case *GameView:
			...
			v.director.window.SetTitle(v.title)
			v.console.SetAudioChannel(v.director.audio.channel)
			v.director.window.SetKeyCallback(
		...
		case *MenuView:
			...
			v.director.window.SetTitle("Select Game")
			v.director.window.SetCharCallback(
...
func playGame(d *Director, path string) {
	...
	setView(d, &GameView{d, console, path, hash, createTexture(), false, nil})
}
